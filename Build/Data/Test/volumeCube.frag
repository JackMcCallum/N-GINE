...
// Noise stuff from http://www.geeks3d.com/20110317/shader-library-simplex-noise-glsl-opengl/
// some great stuff there, check it out!
...
uniform vec3 params; // time/density/edgefalloff


void main()
{
	float alpha = 
		smoothstep(0.0, params.z, 1 - abs(fPosition.x)) *
		smoothstep(0.0, params.z, 1 - abs(fPosition.y)) *
		smoothstep(0.0, params.z, 1 - abs(fPosition.z));
	
	if (alpha == 0)
		discard;
	
	float perlin = 
		(simplexNoise4(vec4(fPosition * 1, params.x)) * 0.5) +
		(simplexNoise4(vec4(fPosition * 3, params.x)) * 0.3);
	...
	oColor = vec4(mix(color1, color2, perlin),params.y * perlin * alpha);
}
